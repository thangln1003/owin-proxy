...
namespace AngularDemo.SpaServices.Proxying
{
    ...
    internal static class SpaProxy
    {
        ...
        public static async Task<bool> PerformProxyRequest(
            HttpContext context, 
            HttpClient httpClient, Task<Uri> baseUriTask, CancellationToken applicationStoppingToken, bool proxy404s)
        {
            // Stop proxying if either the server or client wants to disconnect
            var proxyCancellationToken = CancellationTokenSource.CreateLinkedTokenSource(context.Request.TimedOutToken, applicationStoppingToken).Token;
            ...
            try
            {
                ...
                else
                {
                    ...
                    using (var responseMessage = await httpClient.SendAsync(requestMessage, HttpCompletionOption.ResponseHeadersRead, proxyCancellationToken))
                    {
                        if (!proxy404s)
                        {
        ...
        private static HttpRequestMessage CreateProxyHttpRequest(HttpContext context, Uri uri)
        {
            ...
            if (!string.Equals(requestMethod, "GET") &&
                !string.Equals(requestMethod, "HEAD") &&
                !string.Equals(requestMethod, "DELETE") &&
                !string.Equals(requestMethod, "TRACE"))
            {
            ...
            for (var i = 0; i < headers.Count; i++)
            {
                ...
                {
                    requestMessage.Content?.Headers.TryAddWithoutValidation(headers.GetKey(i), headers.Get(i));
                }
            }
        ...
        private static async Task<bool> AcceptProxyWebSocketRequest(HttpContext context, Uri destinationUri, CancellationToken cancellationToken)
        {
            ...
            {
                var headerEntry = context.Request.Headers;
                for (int i = 0; i < headerEntry.Count; i++)
                //foreach (var headerEntry in context.Request.Headers)
                {
                    if (!NotForwardedWebSocketHeaders.Contains(headerEntry.GetKey(i), StringComparer.OrdinalIgnoreCase))
                    {
                        try
                        {
                            client.Options.SetRequestHeader(headerEntry.GetKey(i), headerEntry.Get(i));
                        }
                ...
                //using (var server = await context.AcceptWebSocketAsync(client.SubProtocol))
                {
